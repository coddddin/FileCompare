...
![과제3 실행화면](img/error3.png)

**✅ 과제 내용**
- `>>>` 및 `<<<` 버튼 클릭 시 선택된 폴더로 파일 복사

- 동일 이름의 파일이 있을 경우 덮어쓰기 허용 여부 확인

- 복사 완료시 목록 갱신하여 변경된 상태 반영


**💡 상세 구현 내용**
- `File.Copy(source, dest, true)` 메서드를 사용하여 동일 이름의 파일이 있을 경우 덮어쓰기가 가능하도록 구현했습니다. 

- `Path.Combine`을 사용하여 경로 문자열을 안전하게 생성하고, 복사가 끝나면 다시 리스트를 불러와 변경된 상태(검은색)를 확인하게 했습니다.

- 복사 과정에서 발생할 수 있는 예외(예: 파일 잠금, 권한 문제)를 `try~catch`로 처리하여 프로그램이 강제 종료되지 않도록 했습니다.

- 복사 후 `RefreshListView()` 메서드를 호출하여 양쪽 폴더의 파일 상태를 다시 비교하고 색상으로 업데이트하도록 구현했습니다.

---

## 📸 과제 4: 하위 폴더 처리 (재귀 탐색)

![과제4 실행화면](img/dir4.png)

------------------------------------------------

![과제4 실행화면](img/lowDir4.png)

- 하위 폴더 더블클릭시 내용 확인( 비교 복사 가능 )
...
- 하위 폴더 더블클릭 시 해당 폴더의 내용 표시 및 비교 기능 유지
	
- 폴더 복사 시에도 하위 폴더까지 포함하여 복사 수행

**💡 상세 구현 내용**
- `GetDirectories()`를 호출하여 하위 디렉토리가 존재할 경우 자기 자신을 다시 호출하는 재귀적 구조를 설계했습니다.

- 이를 통해 폴더 구조가 아무리 깊어도 모든 경로를 탐색하여 파일 비교를 수행할 수 있게 확장했습니다.

- 하위 폴더를 더블클릭하면 해당 폴더의 내용을 `ListView`에 표시하도록 이벤트 핸들러를 추가하여 사용자가 쉽게 탐색할 수 있도록 했습니다.

- 폴더 복사 시에도 `Directory.CreateDirectory`와 `File.Copy`를 조합하여 하위 폴더와 파일을 모두 복사하도록 구현했습니다. 

---

## 💡 실습 소감 및 분석
이번 7주차 실습은 파일 시스템을 직접 제어하고 복잡한 비교 로직을 시각화하는 과정을 다루었습니다. 
단순히 파일을 옮기는 것을 넘어, 알고리즘 설계에 대해 더 고민해 보게 되었습니다.